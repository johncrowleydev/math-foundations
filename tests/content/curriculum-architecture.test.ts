...
import {
  inspectCurriculumArchitecture,
  inspectCurriculumFile,
} from '../../tools/audit/curriculum-architecture.js';
...
test('moving obvious lesson DSLs and banks elsewhere does not evade the boundary', () => {
  for (const source of [
    ...
    assert.match(
      inspectCurriculumFile('tools/content/new-generator.mjs', source).join(),
      /authoring/,
    );
  }
});
...
test('the retired MDX adapter cannot return as a lesson rendering path', () => {
  assert.match(
    inspectCurriculumFile(
      'tools/content/lesson-mdx.ts',
      'export function compileLessonMdx() {}',
    ).join(),
...
test('static curriculum processors cannot become browser rendering dependencies', () => {
  for (const source of [
    ...
    'import { extractLessonMetadata } from "../../tools/content/lesson-metadata";',
    'export { loadContent } from "../../tools/content/content";',
    'const adapter = await import("../../tools/content/lesson-metadata");',
    'const adapter = require("../../tools/content/lesson-metadata");',
  ]) {
    assert.match(
  ...
  for (const [path, source] of [
    ...
    [
      'web/vite.config.ts',
      'import { validateLessonMdx } from "../tools/content/lesson-mdx-policy";',
    ],
  ]) {
...
test('normal rendering, output builds, and synthetic test fixtures remain valid', () => {
  for (const [path, source] of [
    ['tools/content/build.ts', 'await writeFile("output/content/notebook.json", data);'],
    ['tools/content/content.ts', 'const lessons = await readFile("content/curriculum.yaml");'],
    ['web/src/Exercise.tsx', 'export function Exercise(props) { return <p>{props.prompt}</p>; }'],
    ['tests/content/renderer.test.ts', 'const exerciseBank = [{ prompt: "Synthetic example?" }];'],
    ['server/review_test.go', 'const reviewQuestions = [];'],
    ['e2e/review.spec.ts', 'const exerciseBank = [{ prompt: "Synthetic example?" }];'],
  ]) {
    assert.deepEqual(inspectCurriculumFile(path, source), [], path);
  }
});
