...
import { knowledgeExercises } from '../../tools/content/choice-exercises.js';
import { teachingSections } from '../../tools/content/inline-prerequisites.js';
import { placeNotebookExercises } from '../../tools/content/notebook-placements.js';
import { quickChecks } from '../../tools/content/quick-checks.js';
import {
  exerciseKey,
  exerciseNamespace,
...
const plans = await Promise.all(
  // Preserve every published worksheet ID across each lesson split.
  [
    ...
    ...(await json<{ lessons: PlanLesson[] }>(
      `tests/curriculum/fixtures/linear-algebra-${baseline.name}-pacing.json`,
    )),
  })),
);
...
test('split lessons preserve question ownership, teaching order, and inline placements', () => {
