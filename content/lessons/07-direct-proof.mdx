...
Write a few complete proofs carefully instead of treating the full practice collection as a quota. The recommended questions include identifying a proof's obligations, proving a parity claim, constructing a witness, and repairing an invalid argument. The worked examples develop the other patterns; extra practice is available when one of those patterns needs more attention.
...
For a direct implication, a useful draft has four lines: **assume the hypothesis; unpack its definitions; connect those facts to the definition of the goal; conclude the claim.** Leave the middle unfinished at first if necessary. For “an even integer has an even square,” the first line supplies $n=2k$ for an integer $k$, and the goal asks for $n^2=2m$ with $m$ an integer. Substitution gives $n^2=4k^2=2(2k^2)$, so the missing witness is $m=2k^2$. We discovered it by comparing what we had with what the goal required. The result holds because that witness is an integer, not merely because a 2 appears in the expression.
