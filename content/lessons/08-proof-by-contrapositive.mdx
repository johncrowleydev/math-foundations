...
This lesson builds on [Propositional Logic](../lessons/01-propositional-logic.mdx), [Predicates and Quantifiers](../lessons/02-predicates-and-quantifiers.mdx), and [Direct Proof](../lessons/07-direct-proof.mdx). Keep the earlier definitions of even, odd, rational, and divides. Unless another domain is specified, variables in divisibility and parity claims are integers.
...
A useful question is, “Does the negated conclusion give me a concrete representation?” For integer parity, “not even” gives the formula for an odd integer; for a real number, “not irrational” gives a rational fraction. That is often more usable than trying to recover an input from information about its square. Once you have written the new assumption and goal, the remaining work is an ordinary direct proof. If the new assumption is no easier to use, another method may be simpler.
...
The recommended practice checks the transformation, a compound negation, one complete parity proof, and a mistaken argument. Use additional practice for a specific weak point, such as remainder cases or rationality, rather than repeating every complete proof. When checking a solution, compare its starting assumption and final obligation with yours before comparing the algebra.
