...
<Figure id="graph-first" alt="Four vertices joined by four edges" />
...
<Figure id="graph-degree" alt="Count the incident edges at each vertex" />
...
<Figure id="graph-routes" alt="Compare a path, a cycle, and a repeated-edge walk" />
...
<Figure id="graph-components" alt="Two separate connected components" />
...
<Figure id="graph-tree" alt="A connected graph with no cycle" />
...
<Figure id="graph-root" alt="Choose a root and read the parent relationships" />
...
<Figure id="graph-dag" alt="Compare two valid topological orderings" />
...
<Figure id="graph-matrix" alt="Read the graph as a matrix or as neighbor lists" />
...
<Figure id="graph-search" alt="Step through breadth-first and depth-first discovery" />
...
<Figure id="graph-euler-hamilton" alt="A Hamiltonian cycle that leaves edges unused" />
