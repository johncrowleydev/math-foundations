...
To express these precisely, we need **predicates**, which describe properties or relationships, and **quantifiers**, which say how many objects satisfy them. This lesson builds on [Propositional Logic](../lessons/01-propositional-logic.mdx), especially implication, negation, and counterexamples.
...
The recommended practice samples translation, negation, witness choices, and one short proof. Treat the remaining collection as extra practice for the particular idea you need, rather than a requirement to answer every similar question.
...
For a constructive existence proof, use this short skeleton: **name a candidate, check its domain, then verify the property.** For instance, to prove that an integer lies strictly between 6 and 8, choose 7. It is an integer, and $6<7<8$. The sentence “let $n$ be arbitrary” would be the wrong start here: you are allowed to choose a helpful witness, and the claim does not promise that every integer works.
...
<Figure id="quantifier-witness" alt="A finite-domain witness" />
...
<Figure id="quantifier-order" alt="Comparing two quantifier orders" />
...
### Worked example: prove both jobs separately

Prove that there is exactly one integer $n$ satisfying $n+4=10$.

**Existence:** choose $n=6$. It is an integer, and substituting gives $6+4=10$. We have now shown that at least one solution exists.

**Uniqueness:** let $a$ and $b$ be any integers satisfying the equation. Then $a+4=10$ and $b+4=10$, so $a+4=b+4$. Subtracting 4 from both sides gives $a=b$. Any two solutions must therefore be the same integer.

This gives a reusable uniqueness skeleton: **assume two candidates satisfy the property, use that property, and show the candidates are equal.** Do not assume they are equal at the beginning; equality is the goal. Checking that 6 works is enough for existence, but the comparison argument explains why a second solution cannot exist. The same pattern will later prove uniqueness when there is no simple equation to solve.
...
The first words of the claim tell you the first move. For $\forall x\in D$, begin with an arbitrary allowed $x$; for an implication, assume its antecedent; for an existential goal, construct and check a witness. A statement can require these moves in sequence. Draft the opening and the required final sentence before filling in the middle.
...
Why choose $n+1$? The goal says the output must be larger than the supplied input, so adding a positive integer meets both the size requirement and the integer-domain requirement. Choosing $m=100$ would work for some inputs and fail for $n=100$. Choosing $n+1/2$ would always be larger but would fail the required domain. Checking both conditions explains the construction instead of leaving it as a guess.
...
The target “$n^2$ is odd” tells us to seek the form $2m+1$ with $m$ an integer. That is why the calculation factors out 2 and retains a final $+1$. We are using the definition twice: first to turn the hypothesis into something usable, then to recognize when the conclusion has been reached. Starting from “$n^2$ is odd” would assume the very fact still owed.
