...
Use the recommended questions to check the main distinctions. You do not need to complete the entire practice collection before moving on. If a question exposes a confusion, return to the matching example and try a nearby question; repeating every variation is unnecessary when you can already explain the reasoning.
...
**What should you try first?** To refute an implication, deliberately look for a true antecedent and false consequent. To refute an equivalence, look for a case where one side is true and the other false. Those are different targets. For example, making both sides false defeats neither an equivalence nor an argument for equivalence. If your search finds no counterexample, that is a reason to investigate further, not yet a proof: use a complete truth table or a chain of established equivalences to finish.
...
This example previews proof construction. Read it for the method; the dedicated proof lessons later provide the practice. For a direct proof of $P\to Q$, assume $P$, unpack what $Q$ requires, and build justified steps from the assumption to that target. When the negation of $Q$ gives an easier starting point, prove the equivalent implication $\neg Q\to\neg P$ instead. The statement chooses the starting assumption and finishing obligation; we cannot simply assume whichever fact would make the calculation easiest.
