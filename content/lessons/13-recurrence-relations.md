...
This lesson builds on [Sequences and Summations](../lessons/06-sequences-and-summations.md), [Mathematical Induction](../lessons/10-mathematical-induction.md), and [Strong Induction](../lessons/11-strong-induction.md). We will work with integer indices and use algebra and finite sums. No calculus is needed.
...
More generally, if $a_n-a_{n-1}=f(n)$ for $n\geq1$, then
...
with fixed real constants $r,b$ and specified $a_0$. The homogeneous case has $b=0$ and solution $a_n=r^na_0$ for $n\geq1$, with the specified initial value handled separately. This also covers $r=0$ without needing to interpret $0^0$.
...
This is the **characteristic equation**. If it has two distinct real roots $r_1,r_2$, each corresponding geometric sequence satisfies the recurrence, and linearity shows that
...
This models two recursive half-size tasks plus exactly $n$ units of additional work. A recursion tree has $2^j$ nodes of size $n/2^j$ at level $j$. Before the leaves, the level's additional work is
...
Merge sort motivates this model: sort two halves and merge the results with work proportional to the total length. A real implementation's work is usually bounded above and below by constants times $n$, rather than exactly equal to $n$. Such bounds lead to an asymptotic conclusion, not this particular exact operation count.
