...
Validation passed: `npm test` (835 tests, including content/source, deterministic fixtures and generator properties), `npm run typecheck`, `npm run web:build`, `npm run web:test` (77 tests), `npm run format:check`, `go test -count=1 ./...`, and `go test -race -count=1 ./...` (417.5 seconds). The Go runs include deterministic provider traps, frozen-definition/restore tests, and the new location/prerequisite regressions. The effective-catalog preservation report also passes.
