...
| Linear algebra core/pacing JSON                                                                                                | Original seed questions and historical split grouping/IDs                                                                                                          | Existing worksheets and MDX are canonical; split ownership fixtures now live under `tests/curriculum/fixtures/`; the redundant original seed snapshot was removed in the tooling cleanup |
...
| `verify-linear-algebra.py`, `deterministic-adversarial.py`                                                                     | Independent arithmetic verification and grader test-oracle construction                                                                                            | Retained as test tools under `tools/verification/`; neither owns published curriculum                                                                                                    |

Additional executable ownership outside that directory was removed as well:

- The discrete-mathematics section/ID map in `tools/content/notebook-placements.ts`, and
...
validation in `tools/content/lesson-mdx-policy.ts` rejects content-generating JavaScript,
arbitrary imports, spreads, event-handler props, and other disallowed executable
constructs independently of rendering.

`tools/content/lesson-metadata.ts` is a separate static inspection path for section
...
hashes in `tests/content/fixtures/published-compatibility.json`. The notebook comparison
...
fixture `tools/content/compatibility/grading-version.json` maps only this exact proven
...
`tools/verification/verify-linear-algebra.py` reads declarative numerical
fixtures and compiled questions and independently checks their mathematics with
SymPy. `tools/verification/deterministic-adversarial.py` constructs synthetic
grader-test oracles in `tests/grading/fixtures/deterministic-adversarial-corpus.json`; these are
...
After removing the old authoring tree, `npm run content:build` passed, as did all 1,172
...
| `e2e/mdx.spec.ts`                                                        | Passed on the production preview: explicit figures, frame navigation, exercise draft restoration and quick-check selection, with all four subjects and desktop/phone captures.          |
...
| Browser behavior                  | `e2e/mdx.spec.ts` passed against the final production preview after removing legacy prose/figure blocks from notebook JSON. It verifies actual figures and exercises, draft/choice restoration, formula/reference popovers, interactive frames, deep links and saved bookmarks.  |
