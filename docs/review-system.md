...
Authored content stays in Git: dedicated definitions, variants, and generator declarations live in `content/review-templates.json`; reused exercises remain in their existing lesson/worksheet content and evidence mappings. `npm run content` validates and compiles these into `output/grading-catalog.json`. SQLite contains learner/runtime state—attempts, grades, schedules, activations, sessions, and issued instances—not authored review definitions.
