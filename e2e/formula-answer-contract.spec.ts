...
await withBrowser('formula-answer-contract', async ({ browser, baseURL, directory }) => {
  ...
  for (const { name, lesson, question: authoredQuestion, legacy } of cases) {
      ...
      await page.route('**/api/**', async (route) => {
        ...
        if (path.endsWith('/auth/session'))
          ...
          if (question.choice) {
            ...
            assert.ok(selected);
            const verdict = selected.id === question.choice.correctOption ? 'correct' : 'incorrect';
            body = {
              ...attempt,
        ...
        if (formula) {
          ...
          for (const input of assessment.inputs) {
            ...
            if (input.kind === 'math' || input.kind === 'text') {
              ...
              const selected: (string | boolean | null)[] = Array.isArray(value) ? value : [value];
