...
await withBrowser('review-layout', async ({ browser, baseURL, directory }) => {
  for (const viewport of [
    ...
    try {
        ...
        if (path.endsWith('/auth/session')) {
          ...
          body = {
            ...
            targets: instances.map(({ context: item }) => ({
              ...
              dueAt: covered ? now + day : now - day,
              lastEvidenceAt: covered ? now + 5000 : now - day,
      ...
      const start = review.getByRole('button', { name: 'Start Regular review', exact: true });
      ...
      complete = true;
      await refresh();
      await review.getByText('Your plan is complete for now.', { exact: false }).waitFor();
      ...
      await page.waitForFunction(
        () => !document.querySelector<HTMLButtonElement>('.review-page .primary')?.disabled,
      );
      ...
      const cachedNotice = review.getByText('Saved review plan', { exact: false });
      ...
      async function save(status: 'pending' | 'grading' | 'graded') {
        await page.evaluate(
          async ({ item, status }) => {
            const { put } = await import(String('/src/storage.ts'));
            await put('attempts', 'synthetic-layout-answer', {
              id: 'synthetic-layout-answer',
              ...
              ...(status !== 'pending'
                ...
                : {}),
              ...(status === 'graded' ? { verdict: 'incorrect' } : {}),
              grades:
                status === 'graded'
                  ? [
                      {
