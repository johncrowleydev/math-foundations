...
- Only stylus/eraser input makes ink. Exactly two fingers drag or flick to scroll. A new finger or pen contact stops momentum; one finger never starts scrolling. Buttons accept a pen tap. Holding the S Pen side button while writing temporarily selects stroke erasing.
- Thirty optional multiple-choice and true/false checks appear after their teaching sections. Selections and revealed explanations persist, without scores or grading.
...
`scripts/notebook-placements.ts` attaches inline exercises to named teaching headings and stable question IDs. Exercises render after the complete section. Missing or ambiguous headings fail the build instead of silently moving problems when sections are inserted or renamed. `scripts/build-android-content.ts` generates the native asset using this plan. `content/exercise-copy.yaml` owns complete wording for all 1,313 exercises; worksheet files supply stable IDs, formulas, tables, and writing-space sizes. The older `notebook-adaptations.yaml` is historical migration data and is no longer read. `scripts/notebook-exercises.ts` validates exact coverage, rejects printed-context references, and validates the resulting math. The generated asset is not committed. The native rendering test also checks compatibility with JLaTeXMath, including all quick-check choices and explanations.
